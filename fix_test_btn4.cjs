...
// Let's make sure we only replace the first occurrence
let firstOccurrenceIndex = content.indexOf(targetStr);
if (firstOccurrenceIndex !== -1) {
    let before = content.substring(0, firstOccurrenceIndex);
    let after = content.substring(firstOccurrenceIndex + targetStr.length);
    content = before + replaceWith + after;
    fs.writeFileSync('src/pages/admin/ConfiguracaoNotificacoes.tsx', content);
    console.log("Moved button to the correct place!");
} else {
