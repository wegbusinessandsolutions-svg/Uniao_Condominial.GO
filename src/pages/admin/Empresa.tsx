...
export default function Empresa() {
  ...
  const [printingItem, setPrintingItem] = useState<EmpresaData | null>(null);
  ...
  const handlePrint = (item: EmpresaData) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const querySnapshot = await getDocs(collection(db, "config_empresa"));
      const items: EmpresaData[] = [];
      querySnapshot.forEach((doc) => {
        items.push({ id: doc.id, ...(doc.data() as Omit<EmpresaData, "id">) });
      });
      setData(items);
    } catch (error) {
  ...
  const handleOpenModal = (item?: EmpresaData) => {
    setEmailError("");
    if (item) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.email && !emailRegex.test(formData.email)) {
      setEmailError("Formato de e-mail inválido (exemplo: contato@empresa.com.br).");
      setActiveTab("Básico");
      return;
    } else {
      setEmailError("");
    ...
    for (const num of [1, 2, 3, 4]) {
      const cpfVal = formData[`resp${num}Cpf` as keyof EmpresaData] as string;
      if (cpfVal) {
    ...
    try {
      const { db } = await initFirebase();
      const collectionRef = collection(db, "config_empresa");
        ...
        <div className="bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 gap-4 border-b border-slate-200 -mx-6 -mt-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
              Cadastro Empresa Franqueada
            </h1>
            <p className="text-slate-500 text-sm mt-1">
              Cadastro, configurações e informações legais da empresa franqueada.
            </p>
          </div>
          <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                          <div className="md:col-span-3">
                            ...
                            <input
                              ...
                              value={
                                formData[
                                  `resp${num}Nome` as keyof EmpresaData
                                ] as string
                              }
                              onChange={handleChange}
                              className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-brand-dark focus:border-brand-dark sm:text-sm"
                          ...
                          <div className="md:col-span-3">
                            ...
                            <input
                              ...
                              value={
                                (formData[
                                  `resp${num}Cpf` as keyof EmpresaData
                                ] as string) || ""
                              }
                              onChange={handleChange}
                              className={`w-full px-3 py-2 border rounded-md sm:text-sm focus:outline-none focus:ring-2 ${
                                !formData[`resp${num}Cpf` as keyof EmpresaData]
                                  ? "border-slate-300 focus:ring-brand-dark focus:border-brand-dark"
                                  : validarCPF(formData[`resp${num}Cpf` as keyof EmpresaData] as string)
                                  ? "border-green-500 focus:ring-green-500/30"
                                  : "border-red-500 focus:ring-red-500/30"
                              }`}
                            />
                          ...
                          <div className="md:col-span-2">
                            ...
                            <input
                              ...
                              value={
                                formData[
                                  `resp${num}Part` as keyof EmpresaData
                                ] as string
                              }
                              onChange={handleChange}
                              className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-brand-dark focus:border-brand-dark sm:text-sm"
                          ...
                          <div className="md:col-span-2">
                            ...
                            <input
                              ...
                              value={
                                formData[
                                  `resp${num}Funcao` as keyof EmpresaData
                                ] as string
                              }
                              onChange={handleChange}
                              className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-brand-dark focus:border-brand-dark sm:text-sm"
                          ...
                          <div className="md:col-span-2">
                            ...
                            <input
                              ...
                              value={
                                formData[
                                  `resp${num}Tel` as keyof EmpresaData
                                ] as string
                              }
                              onChange={handleChange}
                              className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-brand-dark focus:border-brand-dark sm:text-sm"
          ...
          >
            ...
            <tbody>
              {[1, 2, 3, 4].map((num) => {
                ...
                const nome = printingItem[`${prefix}Nome` as keyof EmpresaData];
                const cpf = printingItem[`${prefix}Cpf` as keyof EmpresaData];
                const part = printingItem[`${prefix}Part` as keyof EmpresaData];
                const func =
                  printingItem[`${prefix}Funcao` as keyof EmpresaData];
                const tel = printingItem[`${prefix}Tel` as keyof EmpresaData];
