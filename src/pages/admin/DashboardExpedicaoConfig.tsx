...
export default function DashboardExpedicaoConfig() {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [statusMsg, setStatusMsg] = useState<{ type: "success" | "error"; text: string } | null>(null);

  const defaultConfig = {
    // Módulos de Logística e Armazém
    moduloEntregas: true,
    moduloEstoque: true,
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const docRef = doc(db, "config", "expedicao_dashboard");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatusMsg({
        type: "success",
        text: "Configurações do Dashboard de Expedição salvas com sucesso!",
      });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2.5">
            ...
            <span>Configurações: Dashboard - Expedição</span>
          </h1>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm md:col-span-2">
          <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Módulos de Logística & Armazém</span>
            </h3>
